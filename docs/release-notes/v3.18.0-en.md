...
>
> CC Switch Remote is a **fully free and open-source** desktop app, and we **do not charge users any fees**. Please only obtain the software through the official channels listed below:
>
> | Channel            | Only Official                                                                  |
> | ------------------ | ------------------------------------------------------------------------------ |
> | Website            | **[github.com/xiaoY233/cc-switch-remote](https://github.com/xiaoY233/cc-switch-remote)**                                         |
> | Source             | **[github.com/xiaoY233/cc-switch-remote](https://github.com/xiaoY233/cc-switch-remote)** |
> | Downloads          | **[GitHub Releases](https://github.com/xiaoY233/cc-switch-remote/releases)**        |
> | Maintainer         | **[@xiaoY233](https://github.com/xiaoY233)**                               |
> | Report an Imposter | **[GitHub Issues](https://github.com/xiaoY233/cc-switch-remote/issues)**            |
>
> **Any "CC Switch Remote" website or client that asks you for payment, top-ups, or login credentials is fake.** If you have been tricked into paying, stop the transaction immediately and file a report through GitHub Issues.

---

## Overview

Both main threads of CC Switch Remote v3.18.0 revolve around xAI Grok. The first is **Grok Build joining the managed apps**: xAI's Grok CLI (live config `~/.grok/config.toml`) becomes the eighth managed app alongside Claude Code, Claude Desktop, Codex, Gemini CLI, OpenCode, OpenClaw, and Hermes — provider add / import / one-click switch, bidirectional MCP and Skills sync, deep-link import, a standalone preset list, and proxy takeover with its own route namespace; the companion "Grok Official" entry supports official-login-state detection and import, and CC Switch Remote never touches the official credentials. The second is **xAI Grok account OAuth sign-in**: device-code authorization replaces the API key, the local proxy injects the access token per request, and on the Claude Code / Claude Desktop side it performs the Anthropic Messages → xAI Responses conversion; the Codex side gets a managed OAuth preset with its own compatibility layer — the ChatGPT-backend-private shapes emitted by codex 0.142+ (namespace tool declarations, private fields) are deterministically flattened and stripped, so the strictly parsing xAI gateway no longer returns 422; API-key users get a separate "xAI (Grok)" native Responses direct-connection preset that goes through no conversion at all.
...
Default Skill repositories were re-seeded on every startup by a "supplement missing defaults" pass, so a default repo you deleted silently returned on the next launch. Seeding is now one-time per database, tracked by a settings flag; databases that already contain repositories at upgrade time get the flag set without any re-seeding, so existing selections are untouched. ([#5356](https://github.com/xiaoY233/cc-switch-remote/pull/5356))

### First-Run Tray Language Follows the System Locale

Before any language was chosen in settings, the tray menu was hardcoded to Simplified Chinese — on English / Japanese / Traditional-Chinese systems the main UI correctly followed the OS locale while the tray disagreed, until the user switched language once. The tray now derives its first-run language from the OS locale with the same precedence rules as the frontend (including `zh-TW` / `zh-HK` / `zh-Hant` → Traditional Chinese); an explicitly chosen language always wins, and unreadable locales fall back to Chinese as before. ([#4355](https://github.com/xiaoY233/cc-switch-remote/pull/4355))
...
**Third-party provider routing**: when CC Switch Remote's local proxy converts and forwards Codex, Claude Desktop, or Grok Build requests to third-party providers, each provider may have different requirements for billing, compliance, and data retention. Read the target provider's terms before use.

By enabling these features, users accept the related risks. CC Switch Remote is not responsible for account restrictions, warnings, or service suspensions caused by using these features.
...
- [#5453](https://github.com/xiaoY233/cc-switch-remote/pull/5453): first-class Grok Build support (the core of the eighth managed app), thanks @YUZHEthefool.
- [#5508](https://github.com/xiaoY233/cc-switch-remote/pull/5508): attach reasoning forward across the Responses→Chat bridge, thanks @ka79376046.
- [#5310](https://github.com/xiaoY233/cc-switch-remote/pull/5310): preserve streamed parallel tool-call identity and order, thanks @SaladDay.
- [#5315](https://github.com/xiaoY233/cc-switch-remote/pull/5315): default Codex tool parameters to an object schema, thanks @Komikawayi.
- [#4706](https://github.com/xiaoY233/cc-switch-remote/pull/4706): normalize tool parameter types for strict OpenAI-compatible providers, thanks @Ryan2128.
- [#5356](https://github.com/xiaoY233/cc-switch-remote/pull/5356): keep deleted default Skill repositories deleted, thanks @allenxu09.
- [#4355](https://github.com/xiaoY233/cc-switch-remote/pull/4355): first-run tray language follows the system locale, thanks @LaiYueTing.
- [#5138](https://github.com/xiaoY233/cc-switch-remote/pull/5138): extend backend CI to Linux / Windows / macOS, thanks @zayokami.
...
Visit [Releases](https://github.com/xiaoY233/cc-switch-remote/releases/latest) and download the build for your system.
...
| `CC-Switch-v3.18.0-macOS.tar.gz` | Archive build for manual installation                 |
