...
CC Switch Remote は `ccswitch://` ディープリンクプロトコルをサポートしており、リンクからワンクリックで設定をインポートできます。
...
CC Switch Remote はオンラインのディープリンク生成ツールを提供しています：

**アクセス先**：[ローカル生成ページ](../../../../deplink.html)
...
CC Switch Remote 公式のオンラインディープリンク生成ツールを使用するとより便利です。
...
1. システムが CC Switch Remote を開くかどうかを確認
2. 確認後、CC Switch Remote が起動
3. インポート確認ダイアログを表示
4. インポートを確認
...
CC Switch Remote のインストール時に `ccswitch://` プロトコルが自動登録されます。
...
インポート前に CC Switch Remote は以下を実行します：
...
**確認事項**：
1. CC Switch Remote がインストールされているか
2. プロトコルが正しく登録されているか
3. リンクの形式が正しいか
