...
## CC Switch Remote のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.cc-switch-remote/`
...
Hermes は YAML 設定を使用します。CC Switch Remote は MCP サーバーを `mcp_servers` に書き込み、編集可能なプロバイダーエントリを `custom_providers` に書き込み、Hermes の `providers` dict にある読み取り専用エントリを読み取り、プロバイダー切り替え時に `model.provider` / `model.default` を更新します。
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（CC Switch Remote の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
CC Switch Remote が設定を変更する際の優先順位：

1. **CC Switch Remote データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（CC Switch Remote がバックフィルする）
- CC Switch Remote の `settings.json`
...
CC Switch Remote v3.7.0 で JSON ファイルから SQLite に移行しました：
