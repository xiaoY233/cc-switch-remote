...
## CC Switch Remote 数据存储

### 存储目录

默认位置：`~/.cc-switch-remote/`
...
Hermes 使用 YAML 配置。CC Switch Remote 将 MCP 服务器写入 `mcp_servers`，将可编辑的供应商条目写入 `custom_providers`，读取 Hermes `providers` 字典中的只读条目，并在切换供应商时更新 `model.provider` / `model.default`。
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 CC Switch Remote 的"供应商"） |
| `env` | 环境变量配置 |
...
CC Switch Remote 修改配置时的优先级：

1. **CC Switch Remote 数据库** - 单一事实源 (SSOT)
...
- CLI 工具的配置文件（会被 CC Switch Remote 回填）
- CC Switch Remote 的 `settings.json`
...
CC Switch Remote v3.7.0 从 JSON 文件迁移到 SQLite：
