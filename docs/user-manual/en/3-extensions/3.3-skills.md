...
CC Switch Remote comes pre-configured with the following GitHub repositories:
...
CC Switch Remote provides powerful search and filter features:
...
- **Automatic backup**: Before deletion, the skill is backed up to `~/.cc-switch-remote/skill-backups/`
- Removes the skill from all app directories (Claude, Codex, Gemini, OpenCode, Hermes)
- Removes the skill from the SSOT directory (`~/.cc-switch-remote/skills/`)
...
CC Switch Remote compares installed skills with the remote repository version using **SHA-256 content hashes**. Whenever the remote has any content changes, the corresponding local skill card automatically shows an "Update available" indicator.
...
2. CC Switch Remote batch-downloads all skills with pending updates
...
| ------------------------ | --------------------------------------------------------------------- |
| **CC Switch Remote built-in**   | Default location `~/.cc-switch-remote/skills/`, managed by CC Switch Remote         |
| **`~/.agents/skills`**   | A shared directory conforming to community agent tool conventions, better for cross-tool collaboration |

### How to Switch

Select the target storage location from the settings or management menu in the Skills panel. The switch **does not lose skill state** — CC Switch Remote smoothly migrates existing skills to the new location.
...
v3.13.0 integrates **skills.sh** public registry search so you can discover community skills directly inside CC Switch Remote.
...
- CC Switch Remote has not finished the latest scan
