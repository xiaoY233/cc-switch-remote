...
CC Switch Remote supports the `ccswitch://` deep link protocol, enabling one-click configuration import via links.
...
CC Switch Remote provides an online deep link generator tool:

**URL**: [local generator page](../../../../deplink.html)
...
Using CC Switch Remote's official online deep link generator tool is more convenient.
...
1. The system asks whether to open CC Switch Remote
2. After confirming, CC Switch Remote opens
...
CC Switch Remote automatically registers the `ccswitch://` protocol during installation.
...
```bash
/usr/bin/open -a "CC Switch Remote" --args --register-protocol
```
...
Before import, CC Switch Remote will:
...
**Check**:
1. Is CC Switch Remote installed
