...
CC Switch Remote for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/xiaoY233/cc-switch-remote/releases).
...
2. Add CC Switch Remote to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.cc-switch-remote/` directory exists
2. Restore from backup: `~/.cc-switch-remote/backups/`
...
**Solutions**:
1. Confirm the file was exported by CC Switch Remote
...
**Windows**:
- Check taskbar settings to ensure the CC Switch Remote icon is not hidden
...
2. Restart the app
3. Delete `~/.cc-switch-remote/settings.json` to reset settings
...
3. Download and install the latest package from the CC Switch Remote Releases page.

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and CC Switch Remote runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, CC Switch Remote covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `ccswitch://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/xiaoY233/cc-switch-remote/issues)
2. Search for similar issues
3. If none found, create a new Issue
4. Provide the following information:
   - Operating system and version
   - CC Switch Remote version
   - Problem description and reproduction steps
   - Error messages (if any)

### Log Files

CC Switch Remote stores logs under its application configuration directory. By default this is `.cc-switch-remote` in your home directory; if you changed the configuration directory in Advanced Settings, use that custom location instead.

When submitting an Issue, attach the files relevant to the problem:

- General, network, or proxy errors: `~/.cc-switch-remote/logs/cc-switch.log` and its rotated files
- Application crashes: `~/.cc-switch-remote/crash.log`, `crash.log.1`, and `crash.log.2`
- The default Windows location is `C:\Users\<username>\.cc-switch-remote\...`
