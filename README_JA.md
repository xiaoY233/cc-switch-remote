...
- `main` は CC Switch Remote の製品ブランチで、アプリケーションリリースに使用します。
- `upstream-main` は上流 CC Switch `main` のミラーで、上流機能の同期基準としてのみ使用します。
- デスクトップアプリとリモート Helper は別々の workflow でリリースします。アプリは tag ベース、Helper は `Remote Helper Artifacts` workflow から `remote-helper-latest` に公開します。
- リモート Helper の asset は `cc-switch-remote-helper-*` プレフィックスを使い、デスクトップ GTK/WebKit に依存しない純粋な CLI バイナリです。
...
[Releases](../../releases) ページから最新版の `CC-Switch-Remote-v{version}-Windows.msi` インストーラー、またはポータブル版 `CC-Switch-Remote-v{version}-Windows-Portable.zip` をダウンロード。

### macOS ユーザー

[Releases](../../releases) から `CC-Switch-Remote-v{version}-macOS.dmg`（推奨）または `.zip` をダウンロード。

> **注意**: 開発者アカウント未登録のため、初回起動時に「開発元を確認できません」と表示される場合があります。一度閉じてから「システム設定」→「プライバシーとセキュリティ」→「このまま開く」をクリックしてください。以降は通常通り起動できます。
> Homebrew パッケージは現在、上流 CC Switch を指しており、この remote fork ではありません。

### Arch Linux ユーザー

現在の AUR パッケージは上流 CC Switch を指しており、この remote fork ではありません。
...
- `CC-Switch-Remote-v{version}-Linux-x86_64.AppImage` / `CC-Switch-Remote-v{version}-Linux-arm64.AppImage`（汎用）
