...
#[test]
fn provider_service_switch_codex_keeps_auth_absent_and_writes_provider_bearer() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    enable_codex_official_auth_preservation();
    let _home = ensure_test_home();
...
"#
    ...
    assert!(
        !cc_switch_lib::get_codex_auth_path().exists(),
        "third-party switches must not create auth.json even when preservation is enabled"
    );
    let live_config =
        std::fs::read_to_string(cc_switch_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
        live_config.contains("experimental_bearer_token = \"third-party-key\""),
        "the third-party key must be written to the active provider table; got:\n{live_config}"
    );
}
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.cc-switch-remote/settings.json
