...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.cc-switch-remote/skills/`
...
pub enum SkillStorageLocation {
    /// CC Switch 管理目录 (~/.cc-switch-remote/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.cc-switch-remote/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.cc-switch-remote/skill-backups/）
...
mod tests {
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        std::os::unix::fs::symlink(
            Path::new("../../.cc-switch-remote/skills/test-skill"),
            &pi_skill,
        )
