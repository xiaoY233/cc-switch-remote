...
impl ProxyService {
    ...
    pub async fn start(&self) -> Result<ProxyServerInfo, String> {
        ...
        let server = ProxyServer::new_with_codex_oauth_manager_and_managed_auth_runtime(
            config.clone(),
            self.db.clone(),
            app_handle,
    ...
    pub async fn update_config(&self, config: &ProxyConfig) -> Result<(), String> {
        ...
        if require_restart {
            ...
            let app_handle = self.app_handle.read().await.clone();
            let new_server = ProxyServer::new_with_codex_oauth_manager_and_managed_auth_runtime(
                new_config.clone(),
                self.db.clone(),
                app_handle,
                self.codex_oauth_manager.clone(),
...
"#;
        ...
        assert!(
            message.contains("写入 Codex 配置失败")
                ...
                // Fork adaptation: the fork cleans up takeover residue when the
                // app's routing is disabled but a live backup remains, before
                // reaching the normal switch/hot-switch path. That cleanup
                // restore also captures the model catalog and surfaces the
                // injected directory as a propagated catalog error. Accepting
                // this fork-specific path preserves the test's intent (catalog
                // write failures must surface, not be swallowed).
                || (message.contains("捕获 Codex 模型目录失败")
                    && message.contains("cc-switch-model-catalog.json")),
