...
"#;
    ...
    async fn update_current_claude_provider_syncs_live_when_proxy_takeover_detected_without_backup()
    {
        ...
        let proxy_base_url = format!("http://127.0.0.1:{}", proxy_info.port);
...
impl ProviderService {
    ...
    fn switch_normal(
        ...
        if use_managed_codex_transaction {
            ...
            if !app_type.is_additive_mode() {
                ...
                if let Err(error) = state.db.set_current_provider(app_type.as_str(), id) {
                    // Failure-preserving: if the DB current-provider commit is
                    // rejected, roll the local settings current back so the UI
                    // does not point at a provider the DB never adopted. Mirrors
                    // the managed-Codex branch above (which rolls back through
                    // managed_codex_transaction_error).
                    let rollback_error = crate::settings::set_current_provider(
                        &app_type,
                        previous_local_current.as_deref(),
                    )
                    .err();
                    return match rollback_error {
                        Some(rollback) => Err(AppError::Message(format!(
                            "更新数据库 current 失败: {error}; 恢复本地 current 失败: {rollback}"
                        ))),
                        None => Err(error),
                    };
                }
    ...
    pub fn import_claude_desktop_providers_from_claude(
        ...
        for provider in claude_providers.values() {
            ...
            if crate::claude_desktop_config::is_compatible_direct_provider(provider)
                && Self::claude_provider_models_are_claude_safe(provider)
            {
                meta.claude_desktop_mode = Some(crate::provider::ClaudeDesktopMode::Direct);
            } else if let Some(routes) = Self::suggested_claude_desktop_routes(provider) {
        ...
        if let Err(error) = state.db.ensure_official_seed_by_id(
            ...
            log::warn!("Failed to ensure claude-desktop-official seed during import: {error}");
        }
    ...
    pub(crate) fn suggested_claude_desktop_routes(
        ...
        fn add_route(
            routes: &mut std::collections::HashMap<
                String,
