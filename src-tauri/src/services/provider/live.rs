...
/// Block on a future from a synchronous context, using the Tauri runtime on
/// desktop and a fresh tokio runtime in the tauri-less remote-helper build so
/// the shared Codex live-auth code path compiles and runs in both targets.
#[cfg(feature = "desktop")]
...
fn block_on<F: std::future::Future>(future: F) -> F::Output {
    tokio::runtime::Runtime::new()
        .expect("failed to create tokio runtime for Codex live-auth")
        .block_on(future)
}
