...
This directory contains the Flatpak manifest (`com.ccswitch.remote`) for CC Switch Remote, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/cc-switch-remote.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.ccswitch.remote.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo CC-Switch-Remote-Linux.flatpak com.ccswitch.remote
```

4) Install and run:

```bash
flatpak install --user ./CC-Switch-Remote-Linux.flatpak
flatpak run com.ccswitch.remote
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.ccswitch.remote.yml` with more precise grants:

```yaml
  - --filesystem=~/.cc-switch-remote:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.ccswitch.remote.yml` accordingly.
