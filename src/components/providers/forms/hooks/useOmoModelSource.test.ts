...
describe("OMO warning target identity", () => {
  it("does not deduplicate equal failures from different targets", () => {
    ...
    const remote = buildOmoWarningSignature("remote:server-1", 0, true, [
      "broken",
    ]);
  ...
  it("is stable for the same target regardless of provider failure order", () => {
    ...
    ).toBe(buildOmoWarningSignature("remote:server-1", 0, false, ["a", "b"]));
  });

  it("does not deduplicate the same failure after a same-id connection change", () => {
    const oldConnection = buildOmoWarningSignature("remote:server-1", 0, true, [
      "broken",
    ]);
    const newConnection = buildOmoWarningSignature("remote:server-1", 1, true, [
      "broken",
    ]);
