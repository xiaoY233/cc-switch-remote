...
vi.mock("@/hooks/useSkills", () => ({
  useInstalledSkills: (target: unknown) => {
  ...
  useBulkToggleSkillApp: (target: unknown) => {
    hookTargets.push(target);
    return {
...
const renderPanel = (target?: typeof remoteTarget) =>
  render(
    <UnifiedSkillsPanel
      onOpenDiscovery={() => {}}
      currentApp="claude"
...
describe("UnifiedSkillsPanel", () => {
  ...
  it("keeps Skill search and bulk controls connected to the selected remote target", async () => {
    installedSkillsMock = [makeInstalledSkill({ id: "remote-alpha" })];
    renderPanel(remoteTarget);
    ...
    expect(hookTargets.length).toBeGreaterThanOrEqual(2);
    expect(hookTargets.every((target) => target === remoteTarget)).toBe(true);
  });
  ...
  it("preserves the discoverable cache key when uninstalling a Skill", async () => {
    installedSkillsMock = [
      makeInstalledSkill({
    ...
    await waitFor(() => {
      expect(uninstallSkillMock).toHaveBeenCalledWith({
        id: "owner/repo:skill-id",
        skillKey: "skill-directory:owner:repo",
      });
    });
  });
