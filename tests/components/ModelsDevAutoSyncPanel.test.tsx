...
function renderPanel(target: ManagementTarget = { type: "local" }) {
  const client = new QueryClient({
    defaultOptions: { queries: { retry: false } },
  });
  return render(
    <QueryClientProvider client={client}>
      <ModelsDevAutoSyncPanel target={target} />
    </QueryClientProvider>,
  );
}
...
describe("ModelsDevAutoSyncPanel", () => {
  ...
  it("reloads the automatic sync config after reading the local pricing file", async () => {
    ...
    expect(getModelPricing).toHaveBeenCalledWith({ type: "local" });
    await waitFor(() => expect(screen.getByRole("switch")).not.toBeChecked());
  });

  it("keeps remote state and actions bound to the selected host", async () => {
    ...
    expect(getModelsDevSyncConfig).toHaveBeenCalledWith(remoteTarget);
    expect(
