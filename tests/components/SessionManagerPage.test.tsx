...
const renderPage = (
  options: { appId?: string; target?: ManagementTarget } = {},
) => {
  const { appId = "codex", target } = options;
  ...
  return {
    ...
    ...render(
      <QueryClientProvider client={client}>
        <SessionManagerPage appId={appId} target={target} />
      </QueryClientProvider>,
    ),
  };
};
...
describe("SessionManagerPage", () => {
  ...
  it("shows source file name in the selected session header", async () => {
    renderPage();
