...
const mocks = vi.hoisted(() => ({
  ...
  toastError: vi.fn(),
  toastSuccess: vi.fn(),
  hookTargets: [] as unknown[],
}));

vi.mock("@/hooks/useMcp", () => ({
  useAllMcpServers: (target: unknown) => {
    mocks.hookTargets.push(target);
    return {
      data: mocks.serversMap,
      isLoading: mocks.isLoading,
    };
  },
  useToggleMcpApp: (target: unknown) => {
    mocks.hookTargets.push(target);
    return {
  ...
  useBulkToggleMcpApp: (target: unknown) => {
    mocks.hookTargets.push(target);
    return {
  ...
  useDeleteMcpServer: (target: unknown) => {
    mocks.hookTargets.push(target);
    return { mutateAsync: mocks.deleteServer };
  },
  useImportMcpFromApps: (target: unknown) => {
    mocks.hookTargets.push(target);
    return { mutateAsync: mocks.importServers };
  },
}));
...
describe("UnifiedMcpPanel", () => {
  ...
  it("keeps search and bulk controls connected to the selected remote target", async () => {
    mocks.serversMap = {
      alpha: makeServer("alpha", { name: "Remote Alpha" }),
    };

    renderPanel(undefined, remoteTarget);
    ...
    expect(mocks.hookTargets.length).toBeGreaterThanOrEqual(5);
    expect(mocks.hookTargets.every((target) => target === remoteTarget)).toBe(
      true,
    );
  });

  it("reports partial bulk-toggle failures without discarding successes", async () => {
    mocks.serversMap = {
      alpha: makeServer("alpha", { name: "Alpha" }),
      beta: makeServer("beta", { name: "Beta" }),
    };
    mocks.bulkToggle.mockResolvedValue({
      succeeded: ["alpha"],
      failed: [{ item: "beta", error: new Error("permission denied") }],
    });
    renderPanel();
    ...
    await waitFor(() =>
      expect(mocks.toastError).toHaveBeenCalledWith("common.bulkToggleFailed", {
        closeButton: true,
      }),
    );
  });
