...
import type { AppId, ManagementTarget, Prompt } from "@/lib/api";
...
const mocks = vi.hoisted(() => ({
  ...
  hookTarget: undefined as unknown,
}));
...
vi.mock("@/hooks/usePromptActions", () => ({
  usePromptActions: (appId: AppId, target: unknown) => {
    mocks.hookTarget = target;
    return {
...
function renderPanel(appId: AppId = "claude", target?: typeof remoteTarget) {
  return render(
    <PromptPanel
      open
      appId={appId}
...
describe("PromptPanel", () => {
  beforeEach(() => {
    ...
    mocks.hookTarget = undefined;
  });

  it("keeps prompt search and actions connected to the selected remote target", async () => {
    renderPanel("claude", remoteTarget);
    await waitForPanelReady();

    expect(
      screen.getByRole("textbox", { name: "prompts.searchAriaLabel" }),
    ).toBeInTheDocument();
    expect(mocks.hookTarget).toBe(remoteTarget);
  });

  it("reloads when credentials change for the selected remote profile", async () => {
    const nextTarget = {
      ...remoteTarget,
      secret: "secret-b",
    } as unknown as ManagementTarget;
    const view = renderPanel("claude", remoteTarget);
    ...
    await waitFor(() => expect(mocks.reload).toHaveBeenCalledTimes(1));
    expect(mocks.hookTarget).toBe(nextTarget);
  });
