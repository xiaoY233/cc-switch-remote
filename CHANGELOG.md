...
- **Remote Codex OAuth routing resolves remote tokens**: Remote routing now injects the helper-owned managed auth runtime into the proxy forwarder, so Claude Code requests routed through a remote Codex OAuth provider resolve the remote ChatGPT access token instead of failing because the helper has no desktop Tauri `AppHandle`.
- **Remote Claude Code install repairs shell PATH**: After installing Claude Code through the remote helper, CC Switch Remote now detects native installer paths such as `~/.local/bin/claude` and appends the matching user bin directory to common shell startup files when needed. This makes `claude` runnable in SSH shells such as WindTerm without manual `/root/.local/bin/claude`.
- **Remote universal provider edits can keep the existing API key**: Editing an existing remote universal provider now treats a blank API Key field as "keep the current key" instead of blocking save with "please fill API Key".

### Changed

- **Remote auth and routing share the same Rust runtime path**: The remote helper now passes its managed Copilot/Codex OAuth managers into the shared proxy runtime. Desktop local behavior still falls back to the existing Tauri-managed auth state.
- **Target-aware OAuth status checks**: Provider forms now query managed OAuth status against the selected management target, avoiding local/remote login-state mixups.
